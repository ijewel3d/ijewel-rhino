...
namespace RhinoCyclesCore.Database
{
	public class ObjectShaderDatabase : IDisposable
	{
		...
		/// </summary>
		private readonly ConcurrentDictionary<uint, ConcurrentDictionary<Tuple<Guid, int>, bool>> _rhRenderhashMeshids = new ConcurrentDictionary<uint, ConcurrentDictionary<Tuple<Guid, int>, bool>>();
		/// <summary>
		/// Record renderhash for meshid
		/// </summary>
		private readonly ConcurrentDictionary<Tuple<Guid, int>, uint> _rhMeshidRenderhash = new ConcurrentDictionary<Tuple<Guid, int>, uint>();
		/// <summary>
		/// Record renderhash used object id (meshinstanceid)
		/// </summary>
		private readonly ConcurrentDictionary<uint, uint> _rhMeshinstanceRenderhashes = new ConcurrentDictionary<uint, uint>();
		/// <summary>
		/// Record object ids (meshinstanceid) on renderhash
		/// </summary>
		private readonly ConcurrentDictionary<uint, ConcurrentDictionary<uint, bool>> _rhRenderhashObjects = new ConcurrentDictionary<uint, ConcurrentDictionary<uint, bool>>();
		private readonly ConcurrentDictionary<uint, CyclesObjectShader> _rhMeshinstanceMaterialMetadata = new ConcurrentDictionary<uint, CyclesObjectShader>();
		private readonly ConcurrentQueue<CyclesObjectShader> _cqObjectShaderChanges = new ConcurrentQueue<CyclesObjectShader>();
		#endregion
		...
		public void Dispose()
		{
			...
			_rhMeshinstanceMaterialMetadata.Clear();
			ResetObjectShaderChangeQueue();
		}

		public ConcurrentQueue<CyclesObjectShader> ObjectShaderChanges => _cqObjectShaderChanges;

		public void AddObjectShaderChange(CyclesObjectShader change)
		{
			if (change != null) _cqObjectShaderChanges.Enqueue(change);
		}

		public void ApplyStoredMaterialMetadata(CyclesObjectShader change)
		{
			if (change == null) return;

			if (_rhMeshinstanceMaterialMetadata.TryGetValue(change.Id, out var stored))
			{
				if (string.IsNullOrEmpty(change.MaterialName)) change.MaterialName = stored.MaterialName;
				if (string.IsNullOrEmpty(change.LayerName)) change.LayerName = stored.LayerName;
				if (string.IsNullOrEmpty(change.LayerFullPath)) change.LayerFullPath = stored.LayerFullPath;
				if (string.IsNullOrEmpty(change.MaterialSource)) change.MaterialSource = stored.MaterialSource;
			}

			_rhMeshinstanceMaterialMetadata[change.Id] = new CyclesObjectShader(change.Id)
			{
		...
		public void ResetObjectShaderChangeQueue()
		{
			while (_cqObjectShaderChanges.Count > 0)
			{
				_cqObjectShaderChanges.TryDequeue(out _);
			}
		}
