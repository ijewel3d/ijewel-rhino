...
// IJWEL_STREAM: Cycles render settings/runtime are not part of the local stream snapshot path.
...
namespace RhinoCyclesCore.Database
{
	public class ChangeDatabase : ChangeQueue
	{
		...
		/// </summary>
		// IJWEL_STREAM: Shader graph upload database removed; keep object/material hash relations instead.
		...
		/// </summary>
		// IJWEL_STREAM: Cycles lights are not streamed in the minimal mesh/material snapshot.
		...
		/// </summary>
		// IJWEL_STREAM: Cycles background/environment shader database removed.
		...
		protected ObjectDatabase StreamObjectDatabase => _objectDatabase;
		protected ObjectShaderDatabase StreamObjectShaderDatabase => _objectShaderDatabase;
		protected CameraDatabase StreamCameraDatabase => _cameraDatabase;
		protected RenderSettingsDatabase StreamRenderSettingsDatabase => _renderSettingsDatabase;
		protected RenderEngine StreamRenderEngine => _renderEngine;
		protected uint StreamDocumentSerialNumber => _doc_serialnr;
		...
		internal ChangeDatabase(Guid pluginId, RenderEngine engine, CreatePreviewEventArgs createPreviewEventArgs, BitmapConverter bitmapConverter, uint docsrn) : base(pluginId, createPreviewEventArgs)
		{
			...
			_modalRenderer = true;
			_objectShaderDatabase = new ObjectShaderDatabase(_objectDatabase);
			// IJWEL_STREAM: Cycles preview background shader removed.
		...
		protected override void Dispose(bool isDisposing)
		{
			...
			// IJWEL_STREAM: Cycles shader graph database removed.
		...
		/// </summary>
		public virtual void UploadObjectShaderChanges()
		{
			// IJWEL_STREAM: Cycles shader graph upload database removed. Object/material
			// hash relations are recorded in ObjectShaderDatabase during Apply* calls.
		...
		// IJWEL_STREAM: Cycles shader/light update event payload types were removed with the runtime upload path.
		...
		public virtual void UploadGammaChanges()
		{
			if (LinearWorkflowHasChanged)
			{
				...
				// IJWEL_STREAM: Cycles shader/light gamma upload removed with shader/light databases.
		...
		/// </summary>
		public virtual void UploadDynamicObjectTransforms()
		{
			// IJWEL_STREAM: Direct Cycles object transform upload removed.
			/*
		...
		/// Upload mesh changes
		/// </summary>
		public virtual void UploadMeshChanges()
		{
			// IJWEL_STREAM: Direct Cycles mesh upload removed. Mesh data is retained in ObjectDatabase.
			/*
		...
		{
			// IJWEL_STREAM: Direct Cycles mesh data upload removed.
			/*
			// set raw vertex data
		...
		public void ResetChangeQueue()
		{
			...
			ClearLinearWorkflow();
			// IJWEL_STREAM: Cycles background/environment queue removed.
			// _environmentDatabase.ResetBackgroundChangeQueue();
			_cameraDatabase.ResetViewChangeQueue();
			// IJWEL_STREAM: Cycles light/shader queues removed.
			...
			_objectShaderDatabase.ResetObjectShaderChangeQueue();
		}
		...
		public bool HasChanges()
		{
			...
			// IJWEL_STREAM: Cycles light/shader/background queues removed.
			// bool lightChanges = _lightDatabase.HasChanges();
			// bool shaderChanges = _shaderDatabase.HasChanges();
			bool lightChanges = false;
			bool shaderChanges = false;
			bool objectChanges = _objectDatabase.HasChanges() || _objectShaderDatabase.ObjectShaderChanges.Count > 0;
		...
		protected override void ApplyLinearWorkflowChanges(Rhino.Render.LinearWorkflow lw)
		{
			...
			LinearWorkflow = lw;
			// IJWEL_STREAM: Cycles background/environment shader gamma upload removed.
		...
		/// </summary>
		public virtual void UploadClippingPlaneChanges()
		{
			// IJWEL_STREAM: Direct Cycles clipping-plane upload removed.
			/*
		...
		/// </summary>
		public virtual void UploadCameraChanges()
		{
			// IJWEL_STREAM: Direct Cycles camera upload removed.
			/*
		...
		{
			// IJWEL_STREAM: Direct Cycles camera/focal blur upload removed.
			/*
			var scene = _renderEngine.Session.Scene;
			scene.Camera.FocalDistance = fb.FocalDistance;
			var unitscale = (float)RhinoMath.UnitScale(UnitSystem.Millimeters, ModelUnits);
		...
		protected override void ApplyViewChange(ViewInfo viewInfo)
		{
			...
			{
				// IJWEL_STREAM: Cycles wallpaper/background shader upload removed.
				// _environmentDatabase.SetGamma(PreProcessGamma);
		...
		protected override void ApplyMeshChanges(Guid[] deleted, List<CqMesh> added)
		{
			...
			if (_renderEngine.ShouldBreak) return;
				...
				// IJWEL_STREAM: RhinoCyclesData user-data type was removed with the full Cycles runtime.
		...
		// IJWEL_STREAM: Decals are baked into Cycles shaders in the original renderer.
		// The local stream path is keeping mesh data and material render hashes only.
		/*
		...
		*/
		...
		public void HandleMeshData(Guid meshguid, int meshIndex, Rhino.Geometry.Mesh meshdata, MappingChannelCollection mappingCollection, bool isClippingObject, uint linearlightMatId, ccl.Transform t)
		{
			...
			else
			{
				foreach (var mapping in mappingCollection.Channels)
				{
					...
					// IJWEL_STREAM: RhinoCommon 8 ChangeQueue.MappingChannel no longer exposes Channel here.
					...
					RcCore.It.AddLogStringIfVerbose($"\t\tHandleMeshData: mapping {mapping.Mapping.MappingType} {mapping.Mapping} {mapping.Local}");
		...
		public UnitSystem ModelUnits { get; set; }
		...
		protected override void ApplyMeshInstanceChanges(List<uint> deleted, List<MeshInstance> addedOrChanged)
		{
			...
			Parallel.ForEach(realDeleted, d =>
			{
				...
				if (cob != null)
				{
					var delob = new CyclesObject { obid = d, cob = cob };
					_objectDatabase.DeleteObject(delob);
					// IJWEL_STREAM: ccl.ObjectPtr is only useful for Cycles runtime logging.
					// RcCore.It.AddLogStringIfVerbose($"\tDeleting mesh instance {d} (ptr {cob.ObjectPtr})");
					RcCore.It.AddLogStringIfVerbose($"\tDeleting mesh instance {d}");
				}
				else
				{
...
#pragma warning disable CS0618
				...
				// IJWEL_STREAM: Decals are part of the Cycles shader construction path.
				...
				// IJWEL_STREAM: Decal CRC and Cycles shader construction disabled with shader graph upload.
...
#pragma warning disable CS0618
				...
				var ob = new CyclesObject
				{
					...
					LayerFullPath = metadata.LayerFullPath,
					MaterialSource = metadata.MaterialSource,
					// Decals = cyclesDecals
				};
				var oldhash = _objectShaderDatabase.FindRenderHashForObjectId(a.InstanceId);

				HandleShaderChange(a.InstanceId, oldhash, matid, meshid, metadata);
		...
		{
			var doc = RhinoDoc.FromRuntimeSerialNumber(_doc_serialnr);
			var layer = attributes != null && doc != null && attributes.LayerIndex >= 0 && attributes.LayerIndex < doc.Layers.Count
		...
		private struct MaterialMetadata
		{
		...
		{
			HandleShaderChange(obid, oldhash, newhash, meshid, default(MaterialMetadata));
		}

		private void HandleShaderChange(uint obid, uint oldhash, uint newhash, Tuple<Guid, int> meshid, MaterialMetadata metadata)
		{
			...
			_objectShaderDatabase.ApplyStoredMaterialMetadata(shaderchange);
			...
			if (shaderchange.Changed)
			{
				...
				// IJWEL_STREAM: Cycles shader graph upload database removed.
				// _shaderDatabase.AddObjectMaterialChange(shaderchange);
				_objectShaderDatabase.AddObjectShaderChange(shaderchange);
...
#region SHADERS
		...
		// IJWEL_STREAM: Cycles material shader construction removed. Keep material ids
		// and object/material hash relations; the websocket side will decide how to map materials.
		/*
		...
		*/
		...
		private void HandleMaterialChangeOnObject(uint matid, uint obid, Tuple<Guid, int> meshid)
		{
			...
			var material = MaterialFromId(matid);
			var metadata = new MaterialMetadata
			{
				MaterialName = material?.Name
			};

			HandleShaderChange(obid, oldhash, matid, meshid, metadata);
		}
		...
		protected override void ApplyMaterialChanges(List<CqMaterial> mats)
		{
			...
			// IJWEL_STREAM: Cycles material shader creation removed. The loop above
			// already records the object/material hash relation we need.
		...
		/// </summary>
		public virtual void UploadShaderChanges()
		{
			// IJWEL_STREAM: Cycles material shader upload removed.
		...
		private void InitialiseGroundPlane(CqGroundPlane gp)
		{
			...
			// IJWEL_STREAM: Cycles material shader construction removed.
...
#region LIGHT & SUN
		...
		/// </summary>
		public virtual void UploadLightChanges()
		{
			// IJWEL_STREAM: Cycles light database/upload path removed.
		...
		{
			// IJWEL_STREAM: Linear-light material shader construction removed.
		...
		{
			// IJWEL_STREAM: Cycles background light removed.
		...
		{
			// IJWEL_STREAM: Linear lights are part of the Cycles light/render path.
		...
		{
			// IJWEL_STREAM: Cycles sun/light database upload path removed.
		...
		public virtual void UploadEnvironmentChanges()
		{
			// IJWEL_STREAM: Cycles background/environment shader upload removed.
		...
		{
			// IJWEL_STREAM: Direct Cycles session shadow-catcher API removed.
			// if (ob == null) return;
		...
		/// </summary>
		public virtual void UploadObjectChanges()
		{
			// IJWEL_STREAM: Direct Cycles object upload removed. Object records remain in ObjectDatabase.
			/*
			...
			foreach (var ob in _objectDatabase.NewOrUpdatedObjects)
			{
				...
				// IJWEL_STREAM: Cycles shader graph lookup removed. Keep material render hash on CyclesObject.
		...
		{
			// IJWEL_STREAM: Cycles skylight/background upload removed.
		...
		// IJWEL_STREAM: Cycles integrator settings removed.
		...
		protected override void ApplyRenderSettingsChanges(RenderSettings rs)
		{
			if (rs != null)
			{
				// IJWEL_STREAM: Cycles render preset/integrator settings removed.
		...
		public virtual bool UploadIntegratorChanges()
		{
			// IJWEL_STREAM: Cycles integrator upload removed.
		...
		public virtual bool UploadDisplayPipelineAttributesChanges()
		{
			// IJWEL_STREAM: Direct Cycles display/session upload removed.
